<h2 style="margin-bottom:22px;">Compras</h2>

<div class="table-search-wrap">
  <input type="search" class="table-search" id="purchaseSearch" placeholder="Buscar compra...">
  <a class="btn" href="<?= $base ?>/purchases/create">➕ Nueva Compra</a>
</div>

<table id="purchaseTable" class="sales-table">
  <thead>
    <tr>
      <th>ID</th>
      <th>Fecha</th>
    ...
    <?php foreach ($items as $row): ?>
      <tr>
        ...
        <td>
          ...
          $estado = $row['status'];
          if ($estado === 'confirmed') {
            echo '<span class="status status-paid">Confirmada</span>';
          } elseif ($estado === 'draft') {
            echo '<span class="status status-pending">Borrador</span>';
        ...
        <td>
          <div class="actions-btn-group">
            <a class="btn btn-action btn-secondary" href="<?= $base ?>/purchases/edit?id=<?= $row['id'] ?>">✏️ Editar</a>
            <form method="post" action="<?= $base ?>/purchases/destroy" style="display:inline" onsubmit="return confirm('¿Eliminar compra?')">
...
<script>
  // Buscador de tabla para compras
