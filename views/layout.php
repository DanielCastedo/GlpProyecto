...
:root{
  ...
  --sidebar-start: #0f172a;  /* degradado sidebar */
  --sidebar-end: #111827;
  --border: #e5e7eb;
...
.header{
  ...
  z-index: 40; /* por encima del backdrop */
}
.header .brand{
  color:#fff;
...
.hamburger{
  ...
  border: none; background: transparent; font-size: 1.6rem; cursor:pointer; color: var(--brand-700);
  border-radius: 10px;
}
...
.sidebar nav{ padding: var(--space-4); display:flex; flex-direction:column; gap:8px; }
.sidebar nav a{
...
.sidebar nav a:hover{ background: var(--bg-item); color:#fff; transform: translateX(1px); }
.sidebar nav a.active{ background:#fff; color:#111827; }
.sidebar nav a .icon{ width:22px; text-align:center; }
...
.app{
  ...
  display: block;
  padding-left: 264px; /* deja espacio para la sidebar fija en desktop */
}
.main-wrap{ display:flex; justify-content:center; padding: 28px 22px; }
main{
...
.btn-ghost{ background: #eef2ff; color: var(--brand-700); }
.btn-ghost:hover{ background: #e0e7ff; }

.badge{ display:inline-flex; align-items:center; gap:6px; font-size:.78rem; padding: 4px 8px; border-radius: 999px; background:#eef2ff; color: var(--brand-700); }
.badge.success{ background:#ecfdf5; color:#065f46; }
.badge.warn{ background:#fffbeb; color:#92400e; }
.badge.danger{ background:#fef2f2; color:#991b1b; }

.table{
  width:100%; border-collapse:separate; border-spacing:0; margin-top: 6px; font-size:.95rem;
  border:1px solid var(--border); border-radius: 12px; overflow:hidden;
}
.table thead th{
  background:#f8fafc; text-align:left; font-weight:700; color:#334155; padding:12px 12px; border-bottom: 1px solid var(--border);
}
.table tbody td{ padding:12px 12px; border-bottom:1px solid var(--border); color:#374151; }
.table tbody tr:hover{ background:#f9fafb; }

.form-grid{ display:grid; grid-template-columns: repeat(12,1fr); gap:14px; }
.field{ grid-column: span 12; }
@media(min-width:700px){ .field--6{ grid-column: span 6; } .field--4{ grid-column: span 4; } }
.label{ display:block; font-size:.9rem; color:#374151; margin-bottom:6px; }
.input, .select, .textarea{
  width:100%; border:1px solid var(--border); border-radius: 10px;
  padding: 10px 12px; font-size: .97rem; background:#fff;
...
@media (max-width: 900px){
  ...
  .app{ padding-left: 0; }
  .main-wrap{ padding: 18px 12px; }
  main{ padding: 18px 14px; border-radius: 12px; }
